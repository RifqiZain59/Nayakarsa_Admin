        ...
        <form id="add-form" onsubmit="handleAddUser(event)" class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="institution_name" placeholder="Universitas Indonesia" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 transition bg-slate-50">
                </div>
            </div>
    ...
    <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg z-10 overflow-hidden">
        ...
        <form id="edit-form" onsubmit="handleEditUser(event)" class="p-6 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Nama Universitas</label>
                    <input type="text" name="institution_name" id="edit-institution" placeholder="Universitas Indonesia" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#059669] transition bg-white">
                </div>
            </div>
            <div class="bg-emerald-50 rounded-xl p-4 space-y-3 border border-emerald-100">
                <p class="text-xs font-bold text-emerald-700 uppercase tracking-wider flex items-center gap-1.5">
...
async function handleAddUser(event) {
    ...
    try {
        ...
        // Save main user document inside superadmin -> SUPERADMIN_ID -> universitas -> idHash
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                ...
                const ip = await getIpAddress();
                await db.collection('superadmin').doc(SUPERADMIN_ID).collection('logs').add({
                    activity: `Menghapus akun universitas (${email})`,
                    ipAddress: ip,
                    device: navigator.userAgent,
                    timestamp: firebase.firestore.FieldValue.serverTimestamp()
                });
