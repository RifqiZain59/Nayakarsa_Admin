...
export default function DashboardPage() {
  ...
  useEffect(() => {
    // Pastikan cat selalu lowercase untuk menghindari bug huruf besar/kecil
    const cat = (localStorage.getItem('nyk_category') || 'superadmin').toLowerCase();
    setUserCat(cat);
    setLoading(false);
  }, []);
                ...
                <div key={d.m} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px' }}>
                  <div style={{ display: 'flex', gap: '3px', alignItems: 'flex-end', height: '160px' }}>
                    {(userCat === 'superadmin' || userCat === 'perusahaan') &&
                      <div style={{ width: '11px', height: `${d.p}%`, background: 'rgba(79,70,229,.75)', borderRadius: '3px 3px 0 0' }} title={`Perusahaan: ${d.p}`} />}
                    {(userCat === 'superadmin' || userCat === 'sekolah') &&
                      <div style={{ width: '11px', height: `${d.s}%`, background: 'rgba(6,182,212,.75)', borderRadius: '3px 3px 0 0' }} title={`Sekolah: ${d.s}`} />}
                    {(userCat === 'superadmin' || userCat === 'universitas') &&
                      <div style={{ width: '11px', height: `${d.u}%`, background: 'rgba(139,92,246,.75)', borderRadius: '3px 3px 0 0' }} title={`Universitas: ${d.u}`} />}
                  </div>
